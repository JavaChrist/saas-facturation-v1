                  ...
                  <input
                    ...
                    value={(() => {
                      if (newFacture.dateCreation instanceof Date) {
                        const date = newFacture.dateCreation;
                        if (!isNaN(date.getTime())) {
                          return date.toISOString().split("T")[0];
                        }
                      } else if (typeof newFacture.dateCreation === "string") {
                        try {
                          const date = new Date(newFacture.dateCreation);
                          if (!isNaN(date.getTime())) {
                            return date.toISOString().split("T")[0];
                          }
                        } catch (e) {
                        ...
                        typeof (newFacture.dateCreation as FirestoreTimestamp).toDate === "function"
                      ) {
                        try {
                          const date = (newFacture.dateCreation as FirestoreTimestamp).toDate();
                          if (!isNaN(date.getTime())) {
                            return date.toISOString().split("T")[0];
                          }
                        } catch (e) {
                          console.error("Erreur de conversion de date Firestore:", e);
                        }
                      }
